...
COLOR_BOLD_CYAN=""; COLOR_YELLOW=""; COLOR_TTY_RESET=""
COLOR_GREEN=""; COLOR_STDOUT_RESET=""
if [ "$INTERACTIVE" -eq 1 ] && [ -t 4 ] && [ -z "${NO_COLOR+x}" ]; then
  COLOR_BOLD_CYAN=$(printf '\033[1;36m')
  COLOR_YELLOW=$(printf '\033[33m')
  COLOR_TTY_RESET=$(printf '\033[0m')
fi
if [ -t 1 ] && [ -z "${NO_COLOR+x}" ]; then
  COLOR_GREEN=$(printf '\033[32m')
  COLOR_STDOUT_RESET=$(printf '\033[0m')
fi
...
if [ "$INTERACTIVE" -eq 1 ]; then
  if [ "$LOCALE" = "zh-CN" ]; then printf '\n%s%s%s\n\n' "$COLOR_BOLD_CYAN" "安全检查" "$COLOR_TTY_RESET" >&4
  else printf '\n%s%s%s\n\n' "$COLOR_BOLD_CYAN" "Safety check" "$COLOR_TTY_RESET" >&4; fi
...
if [ "$AUDIT_GATE" = "warn" ]; then
  ...
  if [ "$INTERACTIVE" -eq 1 ]; then
    cat "$AUDIT_TEXT" >&4
    printf '\n' >&4
    if [ "$LOCALE" = "zh-CN" ]; then printf '%s%s%s ' "$COLOR_YELLOW" "Worker 使用 --no-skills，但主 Captain 仍可能受这些技能影响。继续？[y/N]：" "$COLOR_TTY_RESET" >&4
    else printf '%s%s%s ' "$COLOR_YELLOW" "Horsepower workers use --no-skills, but the main Captain may still be influenced. Continue? [y/N]:" "$COLOR_TTY_RESET" >&4; fi
...
if [ "$LOCALE" = "zh-CN" ]; then
  printf '\n%s%s%s\n' "$COLOR_GREEN" "Horsepower 安装成功。" "$COLOR_STDOUT_RESET"
  ...
  printf '\n%s%s%s\n' "$COLOR_GREEN" "Horsepower installed successfully." "$COLOR_STDOUT_RESET"
  if [ "$MODEL_SETUP_COMPLETE" -eq 1 ]; then
