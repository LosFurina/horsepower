...
test("preflight enforces semantic release compatibility boundaries", async () => {
  for (const [name, compatibility, expectedExit] of [
    ["boundaries", { node: ">=22.19.0", pi: "0.80.10", openspec: ">=1.6.0 <2.0.0" }, 0],
    ["arbitrary-node", { node: "supported", pi: "0.80.10", openspec: ">=1.6.0 <2.0.0" }, 1],
    ["old-node", { node: ">=22.18.0", pi: "0.80.10", openspec: ">=1.6.0 <2.0.0" }, 1],
...
test("doctor reports malformed managed release topology as actionable installation checks", async () => {
  for (const hostile of [
    ...
    if (hostile === "compatibility") {
      manifest.compatibility = { node: "", pi: "0.80.10", openspec: ">=1.6.0 <2.0.0" };
      await writeFile(manifestPath, JSON.stringify(manifest));
    }
