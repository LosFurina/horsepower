...
**观察性渲染失败：** 若投影构建或 Pi partial-result 回调抛错，Horsepower 继续执行 dispatch，仅在适用时记录有界投递证据，并仍报告真实的子级与父级终态。渲染缺陷不能授权工作、取消 worker、改变 handoff validation，或覆盖 first-terminal-wins 结算。
