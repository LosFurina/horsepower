...
One-shot workers stream bounded, redacted assistant/tool lifecycle updates. Each display includes dispatch name, agent and role, requested/resolved slot, concrete model, thinking level, and handoff mode. Every accepted dispatch settles asynchronously into a stable failure envelope with machine tokens `completed`, `failed`, `canceled`, or `blocked_needs_human`; await settlement and preserve structured identity/error fields, while localizing only human prose. A timeout or missing observation is not completion; managed failures terminalize created handoffs instead of leaving silent orphans. `main_agent` blocks implementation workers unless the user separately authorizes a bounded reviewer.
...
Optional change/dispatch webhooks use an explicit `generic` or `discord` provider. Existing settings without `provider` remain `generic`; Horsepower never guesses from the URL. `generic` preserves canonical JSON and supports HMAC, Bearer, or `none`. Direct Discord incoming webhooks require the `discord` provider with `auth.mode: "none"`, because the webhook URL already carries the credential. Discord delivery sends bounded text plus one status-colored embed with parsed mentions disabled. When authoritative context is available, orderly fields identify the change/campaign/task, agent/worker, slot/model/thinking, work kind/operation, elapsed or last-progress age, failure/diagnostic, and required action; missing values are omitted rather than guessed. It does not add private lifecycle data.
