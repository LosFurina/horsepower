...
export const horsepowerActionSchemas = {
  single: strict({ action: Type.Literal("single"), ...Change, ...ReviewCampaign, handoffMode: HandoffMode, name: Type.String({ minLength: 1 }), agent: Type.String({ minLength: 1 }), modelSlot: Type.String({ minLength: 1 }), task: Type.String({ minLength: 1 }) }),
  parallel: strict({ action: Type.Literal("parallel"), ...Change, ...ReviewCampaign, handoffMode: HandoffMode, tasks: Type.Array(Task, { minItems: 1, maxItems: 8 }) }),
  chain: strict({ action: Type.Literal("chain"), ...Change, ...ReviewCampaign, handoffMode: HandoffMode, tasks: Type.Array(Task, { minItems: 1, maxItems: 8 }) }),
  create: strict({ action: Type.Literal("create"), ...Change, ...ReviewCampaign, handoffMode: HandoffMode, name: Type.String({ minLength: 1 }), agent: Type.String({ minLength: 1 }), modelSlot: Type.String({ minLength: 1 }), brief: Type.Optional(Type.String({ minLength: 1 })) }),
  send: strict({ action: Type.Literal("send"), ...Change, ...ReviewCampaign, handoffMode: HandoffMode, workerId: Type.String({ minLength: 1 }), message: Type.String({ minLength: 1 }), delivery: Type.Optional(Type.Union([Type.Literal("reject"), Type.Literal("followUp")])), wait: Type.Optional(Type.Boolean()), timeoutMs: Type.Optional(Type.Number({ minimum: 1 })) }),
  steer: strict({ action: Type.Literal("steer"), ...Change, ...ReviewCampaign, workerId: Type.String({ minLength: 1 }), message: Type.String({ minLength: 1 }), wait: Type.Optional(Type.Boolean()), timeoutMs: Type.Optional(Type.Number({ minimum: 1 })) }),
