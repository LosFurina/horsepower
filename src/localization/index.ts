...
const en = {
  ...
  "testPlan.intensity": ({ value }) => `Testing intensity: ${value}.`,
  "testPlan.strictness": ({ value }) => `Gate strictness: ${value}.`,
  "testPlan.level": ({ value }) => `Test level: ${value}.`,
  "testPlan.setup": ({ value }) => `Setup and fixtures: ${value}.`,
  "testPlan.action": ({ value }) => `Action or command intent: ${value}.`,
  "testPlan.expectation": ({ value }) => `Expected observable result: ${value}.`,
  "testPlan.failureMeaning": ({ value }) => `Failure meaning: ${value}.`,
  "testPlan.gatePhase": ({ value }) => `Gate phase: ${value}.`,
  "testPlan.gatePass": ({ value }) => `Pass condition: ${value}.`,
  "testPlan.gateWaiver": ({ value }) => `Permitted waiver: ${value}.`,
  "testPlan.confirm": () => "Confirm these exact profiles, test cases, gates, task scope, and execution mode? No option is selected by default.",
  "testPlan.canceled": () => "Test-and-gate confirmation canceled; no campaign was created and any active campaign is unchanged.",
  "testPlan.invalid": ({ code }) => `Test-and-gate plan is invalid (${code}); revise the official OpenSpec design and tasks, then confirm again.`,
  "testPlan.drift": ({ code }) => `Test-and-gate plan changed (${code}); dispatch is blocked until a new campaign is explicitly confirmed.`,
  "testPlan.migration": ({ code }) => `Existing change has no valid test-and-gate plan (${code}). Add a complete ## Test and Gate Plan to the official design, reference its TC-* and G-* IDs from official tasks, then obtain fresh user confirmation; do not fabricate prior confirmation.`,
...
const zhCN: { [K in keyof typeof en]: Formatter } = {
  ...
  "testPlan.intensity": ({ value }) => `测试强度：${value}。`,
  "testPlan.strictness": ({ value }) => `Gate 严格度：${value}。`,
  "testPlan.level": ({ value }) => `测试层级：${value}。`,
  "testPlan.setup": ({ value }) => `前置条件与 fixture：${value}。`,
  "testPlan.action": ({ value }) => `操作或命令意图：${value}。`,
  "testPlan.expectation": ({ value }) => `预期可观察结果：${value}。`,
  "testPlan.failureMeaning": ({ value }) => `失败含义：${value}。`,
  "testPlan.gatePhase": ({ value }) => `Gate 阶段：${value}。`,
  "testPlan.gatePass": ({ value }) => `通过条件：${value}。`,
  "testPlan.gateWaiver": ({ value }) => `允许 waiver 的条件：${value}。`,
  "testPlan.confirm": () => "确认这些精确 profile、测试用例、gate、任务范围和执行模式吗？没有默认选项。",
  "testPlan.canceled": () => "已取消 test-and-gate 确认；未创建 campaign，既有 active campaign 保持不变。",
  "testPlan.invalid": ({ code }) => `Test-and-gate plan 无效（${code}）；请修改官方 OpenSpec design/tasks 后重新确认。`,
  "testPlan.drift": ({ code }) => `Test-and-gate plan 已变化（${code}）；在显式确认新 campaign 前 dispatch 会被阻止。`,
  "testPlan.migration": ({ code }) => `现有 change 缺少有效 test-and-gate plan（${code}）。请在官方 design 添加完整的 ## Test and Gate Plan，在官方 tasks 引用其中的 TC-* 与 G-* ID，然后取得用户的新确认；不得伪造过去的确认。`,
